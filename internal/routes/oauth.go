...
func SetupOAuthRoutes(router *gin.RouterGroup) {
	auth := router.Group("/auth")
	{
		...
		auth.POST("/oauth/link", middleware.AuthMiddleware(), linkOAuthAccount)
		auth.POST("/oauth/unlink", middleware.AuthMiddleware(), unlinkOAuthAccount)
	}
}
...
func SetupSessionRoutes(router *gin.RouterGroup) {
	auth := router.Group("/auth")
	{
		auth.GET("/sessions", middleware.AuthMiddleware(), listSessions)
		auth.DELETE("/sessions/:id", middleware.AuthMiddleware(), revokeSession)
		auth.POST("/sessions/revoke-all", middleware.AuthMiddleware(), revokeAllSessions)
		auth.POST("/sessions/revoke-others", middleware.AuthMiddleware(), revokeOtherSessions)
	}
}
